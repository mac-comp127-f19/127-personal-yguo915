...
public class MessyExpressions {
    ...
    public static boolean isJustScreaming(String text) {
        ...
        String replacedExclamationText = replacedAText.replace("!", "");
        Boolean isJustScreaming = replacedExclamationText.isBlank();
        return isJustScreaming;
    }
    ...
    public static boolean containsOwnLength(String text) {
        Boolean containOwnLength = false;
        int length = text.length();
        for (int i = 0; i < length-1; i++) {
            if (text.substring(i, i + 1).equals(String.valueOf(length)))
                containOwnLength = true;
        }
        return containOwnLength;
    }
}
