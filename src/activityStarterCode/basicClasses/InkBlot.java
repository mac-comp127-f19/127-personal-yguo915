...
public class InkBlot {
    ...
    public void generateInkBlot() {
        RandomWalk walkX = new RandomWalk(150, 290, 10);
        RandomWalk walkY = new RandomWalk(150, 290, 10);
        RandomWalk walkR = new RandomWalk(150, 255, 0);
        RandomWalk walkG = new RandomWalk(150,255, 0);
        RandomWalk walkB = new RandomWalk(150,255,0);

        for (int i = 0; i <= 15000; i++){
            Color color = getColor(walkR, walkG, walkB);
            Line newLine = advance(walkX, walkY);
            Line newLine2 = mirror(newLine);
            newLine.setStrokeColor(color);
            newLine2.setStrokeColor(color);
            canvas.add(newLine);
            canvas.add(newLine2);
        }
